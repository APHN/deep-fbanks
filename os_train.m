...
str = {} ;
str{end+1} = sprintf('%s;', opts.expDir) ;
str{end+1} = sprintf('%s;', opts.suffix) ;
str{end+1} = sprintf('%.1f;', info.test.psAcc*100) ;
...
switch opts.classifier
  ...
  case {'--linear', '--polynomial', '--rbf', '--rf'}
    switch opts.classifier
      case {'--linear', '--polynomial', '--rbf'}
        t = templateSVM('KernelFunction', opts.classifier(3:end)) ;
        Mdl = fitcecoc(psi(:,train)', imdb.segments.label(:,train), ...
            'Learners', t, ...
            'OptimizeHyperparameters', 'auto', ...
            'HyperparameterOptimizationOptions', struct('ShowPlots', false)) ;
      case '--rf'
        Mdl = fitcensemble(psi(:,train)', imdb.segments.label(:,train), ...
            'Method', 'AdaBoostM2', ...
            'OptimizeHyperparameters', 'auto', ...
            'HyperparameterOptimizationOptions', struct('ShowPlots', false)) ;
    end
    [predicions, scores] = predict(Mdl,psi') ;

    info.scores = scores ;
    info.train.psConfusion = confusionmat( imdb.segments.label(:,train), predicions(train)) ;
    info.train.psAcc = sum(diag(info.train.psConfusion)) / sum(sum(info.train.psConfusion)) ;
    info.test.psConfusion = confusionmat( imdb.segments.label(:,test), predicions(test)) ;
    info.test.psAcc = sum(diag(info.test.psConfusion)) / sum(sum(info.test.psConfusion)) ;
end
